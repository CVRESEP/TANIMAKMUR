...
async function updateStatus() {
    console.log('🔄 Memperbarui status Kas Angkutan menjadi DISETUJUI...');
    try {
        const result = await turso.execute("UPDATE kas_angkutan SET status = 'DISETUJUI' WHERE status != 'DISETUJUI' OR status IS NULL");
        console.log(`✅ Berhasil memperbarui ${result.rowsAffected} baris data Kas Angkutan.`);
    } catch (e) {
