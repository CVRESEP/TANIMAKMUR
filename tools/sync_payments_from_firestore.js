...
// Firebase Config (Service Account from migrate_from_firestore.js)
...
async function syncPayments() {
    console.log('🚀 Memulai Sinkronisasi Pembayaran dari Firestore...');

    try {
        // 1. Tambah kolom paymentDate jika belum ada
        try {
            console.log('--- Menambah kolom paymentDate ke tabel orders...');
            await turso.execute('ALTER TABLE orders ADD COLUMN paymentDate TEXT');
            console.log('✅ Kolom paymentDate berhasil ditambahkan.');
        } catch (e) {
            console.log('ℹ️ Kolom paymentDate mungkin sudah ada (diabaikan).');
        }

        // 2. Tarik data pembayaran dari Firestore
        const snapshot = await firestore.collection('pembayaran').get();
        console.log(`📦 Ditemukan ${snapshot.size} data pembayaran di Firestore.`);
        ...
        for (const doc of snapshot.docs) {
            const p = doc.data();
            const pylId = p.nomorPenyaluran || '';
            const payDate = p.tanggal || '';

            if (!pylId) continue;

            // 3. Update data di Turso
            // Kita update tabel orders yang memiliki pylId cocok
            const result = await turso.execute({
                sql: 'UPDATE orders SET status = ?, paymentDate = ? WHERE pylId = ?',
                args: ['LUNAS', payDate, pylId]
            });

            if (result.rowsAffected > 0) {
                updatedCount += result.rowsAffected;
                
                // Juga update status di tabel penyaluran agar konsisten
                await turso.execute({
        ...
        console.log(`Total data terupdate: ${updatedCount}`);
        console.log(`Data tidak ditemukan di Turso: ${missedCount}`);
