...
async function syncKasAngkutan() {
  console.log('🚀 Memulai Sinkronisasi Kas Angkutan dari Firestore (Format Baru)...');
  
  try {
    // 1. Ambil data dari koleksi 'kasAngkutan' di Firestore
    const snapshot = await db.collection('kasAngkutan').get();
    console.log(`📦 Ditemukan ${snapshot.size} data di Firestore.`);

    if (snapshot.empty) {
      console.log('❌ Tidak ada data Kas Angkutan untuk dipindahkan.');
      return;
    }

    // 2. Siapkan data untuk Turso
    let count = 0;
    for (const doc of snapshot.docs) {
      const data = doc.data();
      const id = doc.id;
      
      // Logika khusus untuk memisahkan Masuk dan Keluar dari web lama
      const isMasuk = data.tipePengeluaran === 'PEMASUKAN';
      const isKeluar = data.tipePengeluaran === 'PENGELUARAN';
      
      const date = data.tanggal || new Date().toISOString().split('T')[0];
      const desc = data.keterangan || (data.namaSopir ? `Operasional: ${data.namaSopir}` : "Tanpa Keterangan");
      const masuk = isMasuk ? (parseFloat(data.nominal) || 0) : 0;
      const keluar = isKeluar ? (parseFloat(data.nominal) || 0) : 0;
      
      const branch = data.kabupaten || 'MAGETAN';
      const noDo = data.noDo || '';
      const noPyl = data.nomorPenyaluran || '';
      const sopir = data.namaSopir || '-';
      const status = 'DISETUJUI';

      // Masukkan ke Turso
      await turso.execute({
        sql: `INSERT OR REPLACE INTO kas_angkutan (id, date, desc, masuk, keluar, branch, kabupaten, noDo, noPyl, sopir, status) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [id, date, desc, masuk, keluar, branch, branch, noDo, noPyl, sopir, status]
      });
      count++;
      process.stdout.write(`\r✅ Migrasi: ${count}/${snapshot.size}`);
    }

    console.log(`\n\n🎉 SELESAI! Seluruh data Kas Angkutan (Uang Masuk & Keluar) telah dipindahkan.`);
